...
class OpenRouterClient:
    ...
    def initial_question(
        self, first_item: dict[str, str], company_context: dict[str, str] | None = None
    ) -> str:
        company_instruction = self._company_instruction(company_context)
...
class VoiceInterviewService:
    ...
    def start(
        ...
        warning = None
        try:
            question = self.llm.initial_question(first, company_context)
        except InterviewError:
    ...
    def answer(self, session_id: str, audio_path: str) -> dict[str, Any]:
        ...
        warning = None
        try:
            llm_turn = self.llm.next_question(
                current, transcript, session.answers, pending, session.company_context
            )
